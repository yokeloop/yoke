...
Act as the orchestrator. Coordinate sub-agents and talk to the user.
...
`$ARGUMENTS` — path to a task file, for example `.yoke/ai/86-black-jack-page/86-black-jack-page-task.md`
...
**1.** Write the file `.yoke/ai/<TASK_SLUG>/<TASK_SLUG>-plan.md` using this format:
...
```bash
git add .yoke/ai/<TASK_SLUG>/<TASK_SLUG>-plan.md
...
Send a notification:
`bash ${CLAUDE_PLUGIN_ROOT}/lib/notify.sh --type STAGE_COMPLETE --skill plan --phase Complete --slug "$TASK_SLUG" --title "Plan ready" --body ".yoke/ai/$TASK_SLUG/$TASK_SLUG-plan.md"`
...
- **Run /yoke:do:** call the Skill tool with `/yoke:do` and the argument `.yoke/ai/<TASK_SLUG>/<TASK_SLUG>-plan.md`. Exit.
- **Review via revdiff:** call the Skill tool with `/revdiff` and the argument `--only .yoke/ai/<TASK_SLUG>/<TASK_SLUG>-plan.md`. If the return is non-empty, apply the annotations to the plan file and overwrite. Then exit. (For another pass, the user invokes `/revdiff` manually.)
