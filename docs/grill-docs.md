...
A grilling session that challenges your plan against the project's domain model, sharpens terminology, and updates documentation inline as decisions crystallise. Same interactive interview as `/yoke:grill`, plus it maintains the glossary (`CONTEXT.md`) and architecture decision records (`docs/adr/`).
...
- **Challenges against the glossary** — flags terms that conflict with `CONTEXT.md`.
...
- **Updates `CONTEXT.md` inline** — captures resolved terms as they happen (glossary only, no implementation detail).
...
- `CONTEXT.md` (or per-context files via `CONTEXT-MAP.md`) — the domain glossary. Format: [CONTEXT-FORMAT.md](../skills/grill-docs/reference/CONTEXT-FORMAT.md).
