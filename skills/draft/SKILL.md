...
The optional marking step between grill and do (ADR-0011,
`.yoke/adr/0011-draft-optional-marking-step.md`). A do-shaped run — the same
...
Review the Draft PR on GitHub and comment on it. Re-run `/draft` to re-mark
per the comments, or run `/do <draft-PR-URL>` (or `/do <slug>`) to implement
the Markers and flip the PR to ready.
