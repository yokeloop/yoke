...
No argument — determine slug from the current branch or the latest `.yoke/ai/*/` directory.
...
- From path to task-file: `.yoke/ai/<slug>/<slug>-task.md`
- From the current branch or the latest `.yoke/ai/*/` directory

**2.** Path to task-file: `.yoke/ai/<SLUG>/<SLUG>-task.md`.
...
- `.yoke/ai/<SLUG>/<SLUG>-report.md` — collect KNOWN_ISSUES from Concerns sections and quality review results
- `.yoke/ai/<SLUG>/<SLUG>-fixes.md` — append the list of fixes to KNOWN_ISSUES
...
After writing the report, auto-commit it. If `.yoke/` is gitignored — skip.
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/lib/notify.sh --type STAGE_COMPLETE --skill review --phase Complete --slug "$SLUG" --title "Review complete" --body ".yoke/ai/$SLUG/$SLUG-review.md"
```
