...
description: >-
  ...
  layout, and generation of CLAUDE.md and yoke-context.md. Used when the user
  writes "bootstrap", "set up yoke", "prepare the project", "yoke init",
  "setup yoke", "first run", "connect yoke", "create CLAUDE.md".
---

# Prepare a project for the yoke flow

Act as the orchestrator. Coordinate agents and make decisions via AskUserQuestion. Agents perform all file operations.
...
3. Generate     → CLAUDE.md + yoke-context + .yoke/ skeleton + recommendations
...
2. **yoke-context-generator** (haiku) — read `agents/yoke-context-generator.md`, pass to the agent:
   ...
   - DOC_CONTENT from PROJECT_PROFILE.existing_rules.doc_content
   - DOMAIN_FINDINGS from PROJECT_PROFILE.domain
     Result → YOKE_CONTEXT_FILE (path to .claude/yoke-context.md) and YOKE_SKELETON (the scaffolded `.yoke/` layout). The agent writes `.claude/yoke-context.md` (stack context) AND scaffolds the `.yoke/` skeleton: `.yoke/context.md` (domain glossary seeded from DOMAIN_FINDINGS), `.yoke/journal.md` (empty), `.yoke/ai/.gitkeep`, `.yoke/adr/.gitkeep`.
...
5. **Recommendations:** RECOMMENDATIONS from automation-recommender
6. **Notes:** VERIFY_NOTES (if any after retry)
...
```bash
git add CLAUDE.md .claude/yoke-context.md .yoke/
git commit -m "chore: bootstrap yoke flow context and .yoke/ scaffold"
```

`.yoke/` is committed by default (the convention). If the user chose "only CLAUDE.md" in step 6a (because `.claude/` is ignored):
...
- Paths to files: `CLAUDE.md`, `.claude/yoke-context.md`, `.yoke/` (context.md, journal.md, ai/, adr/)
