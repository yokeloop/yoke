---
title: /yoke:issues
description: "Breaks a plan, spec, or PRD into independently-grabbable GitHub issues using vertical slices (tracer bullets), publishes them in dependency order, and saves a local index in .yoke/ai."
---

Breaks a plan, spec, or PRD into independently-grabbable GitHub issues using vertical slices (tracer bullets), publishes them in dependency order, and saves a local index in .yoke/ai.
...
**Input:** `$ARGUMENTS` (optional) — an issue reference (number, URL, or path) to break down. Otherwise works from the conversation context.
...
[View `SKILL.md` on GitHub →](https://github.com/yokeloop/yoke/blob/main/skills/issues/SKILL.md)
