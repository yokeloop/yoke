...
compact CLAUDE.md section plus a `commit-msg` hook that blocks a malformed
subject and warns — never blocks — on a bodyless feat/fix/refactor. The ticket
cascade keeps its current softness: no ticket found → commit without one, and
the hook does not check tickets. The ban on trailers is narrowed to identity
trailers (`Co-Authored-By`, `Signed-off-by`); decision trailers are the point.
...
by commit type (both manufacture water in trivial commits); a hard-blocking
body hook (the hook cannot judge content, only presence); markdown files as
the memory medium (a parallel artifact tree duplicating what git already
